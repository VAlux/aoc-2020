 ...
 * position at the top-left, check the position that is right 3 and down 1. Then, check the position that is right 3 and
...
def inputToRows(input: List[String], shift: Shift): List[Array[Char]] =
  if shift.down > 1 then
    input.map(_.toCharArray).zipWithIndex.filter((row, index) => index > 0 && index % shift.down == 0).map(_._1)
